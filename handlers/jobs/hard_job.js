...
module.exports.dig = (event, context, callback) => {
  ...
  var args = [
    "process",
    "job",     // controller (singular)
    JSON.stringify(event),      // event
    JSON.stringify(context),    // context
    "handlers/jobs/hard_job.dig" // IE: handlers/controllers/posts.update
  ]
...
module.exports.drive = (event, context, callback) => {
  // Command: bin/jets_wrapper process controller [event] [context] [handler]
  var args = [
    "process",
    "job",     // controller (singular)
    JSON.stringify(event),      // event
    JSON.stringify(context),    // context
    "handlers/jobs/hard_job.drive" // IE: handlers/controllers/posts.update
  ]
  var ruby = spawn("bin/jets_wrapper", args);
...
module.exports.lift = (event, context, callback) => {
  // Command: bin/jets_wrapper process controller [event] [context] [handler]
  var args = [
    "process",
    "job",     // controller (singular)
    JSON.stringify(event),      // event
    JSON.stringify(context),    // context
    "handlers/jobs/hard_job.lift" // IE: handlers/controllers/posts.update
  ]
  var ruby = spawn("bin/jets_wrapper", args);
...
if (process.platform == "darwin") {
  ...
  module.exports.dig(event, context, (error, message) => {
